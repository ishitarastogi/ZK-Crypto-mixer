...
contract AnonASP {
    ...
    Hasher hasher;
    IAnonAadhaarVerifier anonVerifier;

    address public admin;

    constructor(address _hasher, address _anonVerifier) {
        hasher = Hasher(_hasher);
        admin = msg.sender;
        anonVerifier = IAnonAadhaarVerifier(_anonVerifier);
    }
    ...
    function addUser(uint256 _Commitment,uint[2] calldata _pA, uint[2][2] calldata _pB, uint[2] calldata _pC, uint[34] calldata _pubSignals) external {
